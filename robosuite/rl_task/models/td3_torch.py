...
import numpy as np
from rl_task.buffers.buffer import ReplayBuffer
from rl_task.models.networks import ActorNetwork, CriticNetwork

class Agent:
    def __init__(
        self, actor_learning_rate, critic_learning_rate, input_dims, tau, env, checkpoint_dir='tmp/td3',
        ...
        # Default device to GPU; can be overridden after creation
        self.device = T.device('cuda:0' if T.cuda.is_available() else 'cpu')

        # Create networks
        self.actor = ActorNetwork(
            input_dims=input_dims, fc1_dims=layer1_size, fc2_dims=layer2_size,
            n_actions=n_actions, name='actor', learning_rate=actor_learning_rate, checkpoint_dir=checkpoint_dir
        ).to(self.device)

        self.critic_1 = CriticNetwork(
            input_dims=input_dims, fc1_dims=layer1_size, fc2_dims=layer2_size,
            n_actions=n_actions, name='critic_1', learning_rate=critic_learning_rate, checkpoint_dir=checkpoint_dir
        ).to(self.device)
        
        self.critic_2 = CriticNetwork(
            input_dims=input_dims, fc1_dims=layer1_size, fc2_dims=layer2_size,
            n_actions=n_actions, name='critic_2', learning_rate=critic_learning_rate, checkpoint_dir=checkpoint_dir
        ).to(self.device)

        self.target_actor = ActorNetwork(
            input_dims=input_dims, fc1_dims=layer1_size, fc2_dims=layer2_size,
            n_actions=n_actions, name='target_actor', learning_rate=actor_learning_rate, checkpoint_dir=checkpoint_dir
        ).to(self.device)
        
        self.target_critic_1 = CriticNetwork(
            input_dims=input_dims, fc1_dims=layer1_size, fc2_dims=layer2_size,
            n_actions=n_actions, name='target_critic_1', learning_rate=critic_learning_rate, checkpoint_dir=checkpoint_dir
        ).to(self.device)
        
        self.target_critic_2 = CriticNetwork(
            input_dims=input_dims, fc1_dims=layer1_size, fc2_dims=layer2_size,
            n_actions=n_actions, name='target_critic_2', learning_rate=critic_learning_rate, checkpoint_dir=checkpoint_dir
        ).to(self.device)
